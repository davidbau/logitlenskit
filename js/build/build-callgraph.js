...
function extractCallGraph(js) {
  const functions = new Map(); // name -> { calls: Set, loc: number, type: string, line: number, endLine: number }
  const classes = new Map(); // className -> { extends: string|null, methods: Set, loc: number, line: number, endLine: number }
  ...
  function resolveTargets(node, currentClass, scope, currentFunc) {
    ...
    if (node.type === 'Identifier') {
      ...
      if (functions.has(callee)) {
        // If it's a common name (like "last" or "url"), skip
  ...
  for (const [name, data] of classes) {
    ...
    nodes.push({ id: name, type: nodeType, line: data.line, endLine: data.endLine });
  ...
  for (const [name, data] of functions) {
    nodes.push({ id: name, type: data.type, line: data.line, endLine: data.endLine });
...
function getFileAtCommit(commit, filepath, repoRoot) {
  try {
...
function getCoauthorFlags(hash, repoRoot) {
  try {
...
function countTests(hash, repoRoot) {
  try {
    // Get list of test files at this commit
    const files = execSync(
      `git ls-tree -r --name-only ${hash} -- js/tests/`,
      { encoding: 'utf8', maxBuffer: 1024 * 1024, cwd: repoRoot }
    ).trim();
...
function getTestsTreeHash(hash, repoRoot) {
  try {
    return execSync(`git rev-parse ${hash}:js/tests`, {
      encoding: 'utf8',
      maxBuffer: 1024 * 1024,
...
function getCommitHistory() {
  ...
  // Get commits from main branch that touch the source file
  const mainLog = execSync(
    `git log --format="%H|%ad|%s" --date=short --follow -- ${SOURCE_FILE}`,
...
async function main() {
  ...
  const timeline = [];
  const graphCache = new Map();     // jsHash -> parsed graph data
  ...
  for (let i = 0; i < commits.length; i++) {
    ...
    const js = getFileAtCommit(commit.hash, SOURCE_FILE, repoRoot);
    if (!js) continue;

    const coauthors = getCoauthorFlags(commit.hash, repoRoot);
    const jsHash = hashContent(js);

    let graphData = graphCache.get(jsHash);
    if (!graphData) {
      const graph = extractCallGraph(js);
      ...
      graphCache.set(jsHash, graphData);
    }

    // Cache test counts by tests/ tree hash to avoid re-counting unchanged trees
    const testsTreeHash = getTestsTreeHash(commit.hash, repoRoot);
    let testCount;
    if (!testsTreeHash) {
      testCount = 0;
      ...
      testCount = countTests(commit.hash, repoRoot);
