...
test.describe('Auto-Pinning Behavior', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('/pinning-test.html');
    });

    test('default widget has last row pinned', async ({ page }) => {
        const defaultWidget = page.locator('#widget-default');
        // Last row (position 4 for 5 tokens) should have pinned-row class
        const pinnedRows = defaultWidget.locator('.pinned-row');
        await expect(pinnedRows).toHaveCount(1);
        // Verify it's the last position (data-pos="4")
        await expect(pinnedRows.first()).toHaveAttribute('data-pos', '4');
    });

    test('pinnedRows=[] disables auto-pinning', async ({ page }) => {
        const noPinWidget = page.locator('#widget-no-pinning');
        // Should have no pinned rows
        const pinnedRows = noPinWidget.locator('.pinned-row');
        await expect(pinnedRows).toHaveCount(0);
    });

    test('custom pinnedRows overrides default', async ({ page }) => {
        const customWidget = page.locator('#widget-custom-pin');
        // Should have exactly one pinned row at position 1
        const pinnedRows = customWidget.locator('.pinned-row');
        await expect(pinnedRows).toHaveCount(1);
        await expect(pinnedRows.first()).toHaveAttribute('data-pos', '1');
    });

    test('all three widgets have chart containers', async ({ page }) => {
        // All widgets should have chart containers
        await expect(page.locator('#widget-default [id$="_chart_container"]')).toBeVisible();
        await expect(page.locator('#widget-no-pinning [id$="_chart_container"]')).toBeVisible();
        await expect(page.locator('#widget-custom-pin [id$="_chart_container"]')).toBeVisible();
    });
});
